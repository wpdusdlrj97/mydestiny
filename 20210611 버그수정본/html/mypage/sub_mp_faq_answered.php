...
if (!$user_session) {
    ...
    $qry_string = "SELECT * FROM qna_information where reply_status='1' and qna_writer_email='$user_session' ORDER BY qna_date DESC";
                    ...
                    <div class="inner">
                        <div class="tit clear">
                            ...
                            <h2 class="tit-wrap l-inlinebox">
                                ...
                                <a href="./sub_mp_faq_all.php" class="tit ft_b">문의</a>
                            </h2>
                    ...
                    <div class="inner">
                        <div class="mylist-tab">
                            <div class="wrap clear">
                                <a class="tit_txt" href="./sub_mp_faq_all.php">전체</a>
                                <a class="tit_txt on" href="#n">답변 받음</a>
                                    ...
                                    <div class="inner">
                                        <div class="wrap ">
                                            ...
                                            <div class="item btns l-inlinebox">
                                                ...
                                                <span class="btn status l-inlinebox bx-round_l bg_n" style="cursor:pointer;" onclick="qna_delete(<?php echo $qna_id[$x] ?>)">삭제하기</span>
                                            </div>
                                    ...
                                    <div class="inner">
                                        <div class="wrap">
                                            <div class="item thumbnail bx-round_s">
                                                <img src="<?php echo $qna_main_image[$x] ?>" alt="land image"
                                                     class="img_f">
                                            </div>
                                            <div class="item btns l-inlinebox">
                                                <span class="btn status l-inlinebox bx-round_l bg_n">답변 등록됨</span>
                                                    ...
                                                    <div class="forms">
                                                        <div class="imgs">
                                                            ...
                                                            for($i=0; $i<count($qna_sub_image_list_array); $i++){ ?>
                                                                <img class="l-inlinebox bx-round_l" src="<?php echo $qna_sub_image_list_array[$i]?>" alt="land image">
                        ...
                        <?php }else{  ?>
                            <div style="margin-top:300px;width: 100%; height: 500px; font-size: 18px; text-align: center;">답변 받은 문의내역이 없습니다</div>
                        <?php } ?>
                    </div>
                </div>
            </div>
                        ...
                        <div class="pagination l-fleft">
                            <?php if ($page > 0) { ?>
                                <button class="btn btn_prev" type="button"
                                        onclick="location.href='sub_mp_faq_answered.php?page=<?php echo $page - 1 ?>'"></button>
                            <?php } ?>
                            <div class="wrap">
                                <?php
                                if ($all_count > 0) {
                                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                                        if ($x == $page) { ?>
                                            <a class="btn bx-round_xs page_num on"
                                               onclick="location.href='sub_mp_faq_answered.php?page=<?php echo $x ?>'"><?php echo $x + 1 ?></a>
                                        <?php } else { ?>
    ...
    <script>
        function qna_delete(qna_id) {
            if (confirm("정말로 삭제하시겠습니까?")) { //예 클릭시

                $.ajax({
                    type: "POST"
                    , url: "/server/qna_delete_server.php"
                    , data: {id: qna_id}
                    , success: function (result) {
                        if (result == 'success') {
                            location.href = 'https://landmarking.co.kr/html/mypage/sub_mp_faq_answered.php';
                            alert("해당 문의가 삭제되었습니다.");
                        } else {
                            alert("해당 문의 삭제에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                        }
                    }
                    , error: function () {
