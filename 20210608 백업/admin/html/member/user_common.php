...
if (!$admin_session) {
    ...
    $qry_string = "SELECT * FROM user_information where user_status='0'";
    ...
    while ($row = mysqli_fetch_array($qry)) {
        ...
        array_push($user_latest_date, date('Y.m.d', strtotime($row['user_latest_date'])));
                ...
                <div class="inner">
                    ...
                    <div class="contents has-pg clear">
                        <div class="search-area">
                            ...
                            <a href="/admin/html/member/user_join.php" class="btn btn_join">회원등록</a>
                        </div>
                        <?php if ($count == 0) { ?>
                                ...
                                <table class="table">
                                    ...
                                    <?php for ($x = 0; $x < $count; $x++) { ?>
                                        <tr class="tr has-link"
                                            onClick="location.href='/admin/html/member/user_common_detail.php?id=<?php echo $user_email[$x] ?>'">
                                            ...
                                            <td class="td t-date t-date_latest"><span class="value">
                                                    ...
                                                    <?php
                                                    if ($user_latest_date[$x]=='1970.01.01') {
                                                        echo '접속 기록 X';
                                                    } else {
                                                        echo $user_latest_date[$x];
                                                    } ?>
                                                </span>
                                            </td>
                                            <td class="td t-date t-date_join"><span
                                                        class="value"><?php echo $user_join_date[$x] ?></span></td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
                                ...
                                <?php if ($page > 0) { ?>
                                    ...
                                    <?php if ($option) { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='user_common.php?page=<?php echo $page - 1 ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"></button>
                                    <?php } else { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='user_common.php?page=<?php echo $page - 1 ?>'"></button>
                                    <?php } ?>
                                ...
                                if ($all_count > 0) {
                                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                                        if ($x == $page) {
                                            ...
                                            if ($option) { ?>
                                                <a class="num on"
                                                   onclick="location.href='user_common.php?page=<?php echo $x ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"><?php echo $x + 1 ?></a>
                                            <?php } else { ?>
                                                <a class="num on"
                                                   onclick="location.href='user_common.php?page=<?php echo $x ?>'"><?php echo $x + 1 ?></a>
                                            <?php }
    ...
    <script>
        ...
        function search() {
            ...
            if (select_search == '아이디') {
                location.href = "user_common.php?option=user_email&search=" + user_search_input;
            } else if (select_search == '실명') {
                location.href = "user_common.php?option=user_name&search=" + user_search_input;
            } else { //전화번호
                location.href = "user_common.php?option=user_phone&search=" + user_search_input;
            }
