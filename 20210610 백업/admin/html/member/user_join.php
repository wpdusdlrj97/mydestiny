...
<script>
    ...
    function reset_data(){
        location.href='https://landmarking.co.kr/admin/html/member/user_join.php'
    }
    ...
    function user_join(){
        ...
        if(join_name==''){
            ...
            $.ajax({
                ...
                ,data: {type:'user',name:join_name,nickname:join_nickname,id:join_id,phone:join_phone,password:join_password}
                ,success:function(result){
                    if(result=='success'){
                        alert('일반 회원을 등록하였습니다.');
                        location.href='<?php echo "https://landmarking.co.kr/admin/html/member/user_common.php"?>';
                    }else if(result=='overlap'){
                        alert("중복되는 이메일이 존재합니다");
                    }else{
                        alert("일반 회원 등록에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                    }
                }
                ,error:function(){
                    alert("잠시 후에 다시 시도해주세요");
