                            ...
                            <div class="form-wrap">
                                <form onsubmit="return false;">
                                    <div class="wrap">
                                        ...
                                        <input type="email" id="update_id" value="<?php echo $row_admin['admin_email']?>" readonly>
                                    </div>
                                    <div class="wrap">
                                        <label for="userNick">닉네임</label>
                                        <input type="text"  id="update_nickname" minlength="2" maxlength="6" value="<?php echo $row_admin['admin_nickname']?>" placeholder="변경하실 닉네임을 입력해주세요 (2~6자리)">
                                    </div>
                                    <div class="wrap">
                                        <label for="userName">실명</label>
                                        <input type="text" id="update_name" minlength="2" maxlength="4" value="<?php echo $row_admin['admin_name']?>" placeholder="성명(실명)을 기입해주세요">
                                    </div>
                                    <div class="wrap">
                                        <label for="userTel">휴대전화</label>
                                        <input type="number" id="update_phone" minlength="11" maxlength="11" value="<?php echo $row_admin['admin_phone']?>" placeholder="변경할 휴대전화 번호 (-제외)를 입력해주세요. ">
                                    </div>
                                    <div class="btn-wrap">
                                        <button type="button" class="btn btn_del btn_modal" onclick="delete_user();">회원 삭제</button>
                                        <button type="button" class="btn btn_cancel" onClick="window.open('/admin/html/member/admin_change_pw.php?id=<?php echo $row_admin['admin_email']?>','pop','width=425, height=540, top=200, left=200');">비밀번호 변경</button>
    ...
    <script>
        ...
        function delete_user() {
            ...
            if (confirm("해당 유저를 삭제하시겠습니까?")) {
                ...
                user_email= "'" +"<?php echo $row_admin['admin_email']?>"+"'";

                $.ajax({
                    type: "POST"
                    , url: "/admin_server/user_delete_server.php"
                    , data: {delete_type: 'admin', delete_list: user_email}
                    , success: function (result) {

                        if (result == 'success') {
                            alert("해당 관리자를 삭제하였습니다");
                            location.href = "https://landmarking.co.kr/admin/html/member/user_admin.php";
                        } else {
                            alert("해당 관리자 삭제에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                        }
                    }
                    , error: function () {
            ...
            if (confirm("해당 유저의 정보를 수정하시겠습니까?")) {
                ...
                if(user_name==''){
                    ...
                    $.ajax({
                        ...
                        , data: {type:'admin',email:user_email,name:user_name,nickname:user_nickname,phone:user_phone}
                        , success: function (result) {

                            if (result == 'success') {
                                alert("해당 관리자 정보를 수정하였습니다");
                                location.href = "https://landmarking.co.kr/admin/html/member/user_admin.php";
                            } else {
                                alert("해당 관리자 정보 수정에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                            }
                        }
                        , error: function () {
                            alert("잠시 후에 다시 시도해주세요");
