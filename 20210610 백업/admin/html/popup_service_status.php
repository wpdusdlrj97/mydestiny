...
if (!$admin_session) {
                ...
                <div class="inner">
                    ...
                    <div class="table-area">
                                ...
                                <td class="td t-status" onClick="event.cancelBubble=true; return false;">
                                    <form onsubmit="return false;">
                                        <fieldset class="select-box is-table">
                                            ...
                                            <?php if($row_pay['service_status']=='1'){?>
                                            ...
                                            <?php }else if($row_pay['service_status']=='2'){  ?>
                                                <input type="text" class="select" value="진행중" onfocus="this.blur();" readonly >
                                                <div class="options"  id="service_status_option">
                                            ...
                                            <?php }else{ ?>
                                                <input type="text" class="select" value="완료" onfocus="this.blur();" readonly >
                                                <div class="options"  id="service_status_option">
                                                    <span class="option" data-option="대기중">대기중</span>
                                                    <span class="option" data-option="진행중">진행중</span>
                        ...
                        <div class="btn-wrap">
                            ...
                            <button type="button" class="btn btn_sm" onclick="service_status_update()">확인</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function service_status_update() {
            ...
            var rg_service_type = '<?php echo $row_pay['service_type'];?>';
            var rg_land_id = '<?php echo $row_pay['land_id'];?>';

            var service_status_search =  $('#service_status_option').children(".is-selected").text();
            var rg_service_status;
            ...
            //alert(rg_pay_id+"/"+rg_land_id+"/"+rg_service_type+"/"+rg_service_status);


            $.ajax({
                type: "POST"
                , url: "/admin_server/service_update_server.php"
                , data: {type:'update', pay_id: rg_pay_id, land_id: rg_land_id, service_type: rg_service_type, service_status: rg_service_status}
                , success: function (result) {

                    if (result == 'success') {
                        alert("유료서비스 진행 상황을 수정하였습니다");
                        opener.parent.location.reload();
                        window.close();
                    } else {
                        alert("유료서비스 진행 상황 수정에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                    }
                }
                , error: function () {
