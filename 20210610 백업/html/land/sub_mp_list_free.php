...
if (!$user_session) {
    ...
    $qry_string = "SELECT * FROM land_information where land_register_id='$user_session' and land_free_analysis_status='2' ORDER BY land_register_date DESC";
    ...
    //페이징을 위한 페이지 수 계산
    $page_count = (int)($all_count / 3);
    //현재 페이지에서 나열할 글 수
    $count = 0;

    if ($all_count - ($page * 3) >= 3) {
        $count = 3;
    } else {
        $count = $all_count - ($page * 3);
    }
    ...
    $qry_string = $qry_string . " LIMIT " . ($page * 3) . ", 3";
    $qry = mysqli_query($connect, $qry_string);
                    ...
                    <div class="inner">
                        <div class="mylist-tab">
                            <div class="wrap clear">
                                ...
                                <a class="tit_txt on" href="#">무료 분석 완료</a>
                        ...
                        <div class="pagination l-fleft">
                            <?php if ($page > 0) { ?>
                                <button class="btn btn_prev" type="button"
                                        onclick="location.href='sub_mp_list_free.php?page=<?php echo $page - 1 ?>'"></button>
                            <?php } ?>
                            <div class="wrap">
                                <?php
                                if ($all_count > 0) {
                                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                                        if ($x == $page) { ?>
                                            <a class="btn bx-round_xs page_num on"
                                               onclick="location.href='sub_mp_list_free.php?page=<?php echo $x ?>'"><?php echo $x + 1 ?></a>
                                        <?php } else { ?>
