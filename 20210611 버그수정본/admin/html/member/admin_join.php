                ...
                <div class="contents">
                    <div class="form-wrap">
                        <form onsubmit="return false;">
                            ...
                            <div class="btn-wrap">
                                <button type="button" class="btn btn_cancel"  onclick="reset_data()">취소</button>
                                <button type="button" class="btn btn_sm" onclick="admin_join()">확인</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    ...
    function email_overlap(){
        ...
        if(join_email==''){
            ...
            $.ajax({
                ...
                ,data: {type:'admin',email:join_email}
    ...
    function reset_data(){
        location.href='https://landmarking.co.kr/admin/html/member/admin_join.php'
    }
    ...
    function admin_join(){
        ...
        if(join_name==''){
            ...
            $.ajax({
                type: "POST"
                ,url: "/admin_server/join_server.php"
                ,data: {type:'admin',name:join_name,nickname:join_nickname,id:join_id,phone:join_phone,password:join_password}
                ,success:function(result){
                    if(result=='success'){
                        alert('관리자 회원을 등록하였습니다.');
                        location.href='<?php echo "https://landmarking.co.kr/admin/html/member/user_admin.php"?>';
                    }else if(result=='overlap'){
                        alert("중복되는 이메일이 존재합니다");
                    }else{
                        alert("관리자 회원 등록에 실패하였습니다, 잠시 후에 다시 시도해주세요");
                    }
                }
                ,error:function(){
                    alert("잠시 후에 다시 시도해주세요");
