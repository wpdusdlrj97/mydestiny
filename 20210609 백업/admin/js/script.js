;(function(){
    ...
    function selectBoxFn(){
        ...
        var initSelectFn = function(event){
            event.preventDefault();
            onSelectFn(event.target);
        };

        var onSelectFn = function(targetSelect){
            var $onSelect = targetSelect;
            if($onSelect.classList.contains('on')) offSelectFn($onSelect);
            else {
                handleSelectArea($onSelect);
                offAllSelectFn();
                $onSelect.classList.add('on');
                onOptionFn($onSelect);
                // 외부 클릭 시 해제
                handleSelectArea($onSelect);
            }
        ...
        var offAllSelectFn = function(){
            var $currentSelect = document.querySelectorAll('.select.on');
            $currentSelect.forEach(function(el){
        ...
        var handleSelectArea = function(targetSelect){

            var $clickedSelect = targetSelect;

            var checkAreaFn = function(e){
                e.preventDefault();
                if(e.target.classList.contains('select')) return;
                else{
                    offSelectFn($clickedSelect);
                    removeCheckFn();
                }
            };

            var addCheckFn = function(){
                $area.addEventListener('click', checkAreaFn);
            }

            var removeCheckFn = function(){
                $area.removeEventListener('click', checkAreaFn);
            };

            if($clickedSelect.classList.contains('on')) addCheckFn();
            else removeCheckFn();

        };

        select.forEach(function(el){
            el.addEventListener('click', initSelectFn);
        });
