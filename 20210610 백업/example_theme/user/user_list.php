...
if (!$page) { $page = 0; }
$order = $_GET['order'];
if (!$order) { $order = 'user_join_date';}
$search = $_GET['search'];
$search =  addslashes($search);
$option = $_GET['option'];

$qry_string = "SELECT * FROM user_information where (user_status=0 or user_status=1) ";
...
while ($row = mysqli_fetch_array($qry)) {
    ...
    array_push($user_status, $row['user_status']);
}
...
<head>
    ...
    <style>
        ...
        /*페이징*/
        #user_list_table_pagination_before, #user_list_table_pagination_next {display: inline-block;vertical-align: middle;width: 10px;height: 20px;padding: 6px 11px;margin: 0 10px;cursor: pointer;}
        .user_list_table_pagination_page {display: inline-block;vertical-align: middle;width: 30px;height: 30px;border: solid 1px #D0D2D5;text-align: center;line-height: 30px;color: #000000;font-size: 16px;margin: 0 10px;cursor: pointer;background-color: #FFFFFF;}
...
<div id="contents_box">
    <div id="main_contents_box">
        <div id="main_contents_box_place">
            ...
            <div id="main_contents_box_place_title">유저 리스트</div>
            ...
            <div id="main_contents_box_place_search_box">
                <div id="main_contents_box_place_search_select">
                    <select id="user_list_search" class="search_select">
            ...
            <div id="main_contents_box_place_order_select">
                ...
                <select id="user_list_order" class="order_select" onchange="order()">
            ...
            <div id="main_contents_box_place_table">
                ...
                <table id="destiny_table">
                    ...
                    <?php for ($x = 0; $x < $count; $x++) { ?>
                        <tr id='destiny_click_table' onclick="location.href='user_detail.php?user_email=<?php echo $user_email[$x]?>'">
                            <td><?php echo $x+$page*15+1 ?></td>
                            <td><?php echo $user_email[$x]?></td>
                            <td><?php echo $user_name[$x]?></td>
                            <td><?php echo $user_nickname[$x]?></td>
                            <td><?php echo $user_phone[$x]?></td>
                            <td><?php echo number_format($user_counsel_count[$x]).'건' ?></td>
                            <td><?php echo number_format($user_total_point[$x]).'P' ?></td>
                            <td><?php echo $user_join_date[$x] ?></td>
                            <?php if($user_status[$x]==0){ ?>
                            <td style="color: black; font-weight: bold">사용가능</td>
                            <?php  }else if($user_status[$x]==1){ ?>
            ...
            <div id="main_contents_box_place_pagination">
                <?php if ($page > 0) { // 1페이지가 아닐 경우
                     if ($option) { ?>
                        <img id="user_list_table_pagination_before"
                            src="/admin/image/index_before.png"
                            onclick="location.href='user_list.php?page=<?php echo $page - 1 ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'">
                     <?php } elseif($order) { ?>
                         <img id="user_list_table_pagination_before"
                              src="/admin/image/index_before.png"
                              onclick="location.href='user_list.php?page=<?php echo $page - 1 ?>&order=<?php echo $order ?>'">
                     <?php } else { ?>
                        <img id="user_list_table_pagination_before"
                            src="/admin/image/index_before.png"
                            onclick="location.href='user_list.php?page=<?php echo $page - 1 ?>'">
                     <?php }
                }
                ...
                if ($all_count > 0) {
                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                        if ($x == $page) {
                            if ($option) { ?>
                            <div class="user_list_table_pagination_page pagination_selected"
                                 onclick="location.href='user_list.php?page=<?php echo $x ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"><?php echo $x + 1 ?></div>
                            <?php } elseif($order) { ?>
                                <div class="user_list_table_pagination_page pagination_selected"
                                     onclick="location.href='user_list.php?page=<?php echo $x ?>&order=<?php echo $order ?>'"><?php echo $x + 1 ?></div>
                            <?php } else { ?>
                                <div class="user_list_table_pagination_page pagination_selected"
                                     onclick="location.href='user_list.php?page=<?php echo $x ?>'"><?php echo $x + 1 ?></div>
                            <?php }

                        } else {
                            if ($option) { ?>
                                <div class="user_list_table_pagination_page"
                                     onclick="location.href='user_list.php?page=<?php echo $x ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"><?php echo $x + 1 ?></div>
                            <?php } elseif($order) { ?>
                                <div class="user_list_table_pagination_page"
                                     onclick="location.href='user_list.php?page=<?php echo $x ?>&order=<?php echo $order ?>'"><?php echo $x + 1 ?></div>
                ...
                if ($page + 1 != $page_count && $page_count > 0) {
                    ...
                    if ($option) { ?>
                        <img id="user_list_table_pagination_next"
...
<script>
    ...
    function search(){
        ...
        var user_list_search = document.getElementById("user_list_search");
        //검색 옵션
        var select_search = user_list_search.options[user_list_search.selectedIndex].text;
        //검색어
        var user_search_input = document.getElementById("user_search_input").value;

        if(select_search=='이메일'){
            location.href = "user_list.php?option=user_email&search="+user_search_input;
        }else if(select_search=='닉네임'){
            location.href = "user_list.php?option=user_nickname&search="+user_search_input;
        }else{ //이름
            location.href = "user_list.php?option=user_name&search="+user_search_input;
        }
    ...
    // 유저리스트 정렬 함수
    function order(){
        var user_list_order = document.getElementById("user_list_order");
        var select_order = user_list_order.options[user_list_order.selectedIndex].text;

        if(select_order=='상담 건수순'){
            location.href = "user_list.php?order=user_counsel_count";
        }else if(select_order=='구매 포인트순'){
            location.href = "user_list.php?order=user_total_point";
        }else{ //상담건수
            location.href = "user_list.php?order=user_join_date";
        }
