...
(function ($) {
  ...
  function headerFn() {
    var $header = $('#header'),
        ...
        $gnbBtn = $('#header #navBtn'); // 모바일 모드 전환    
    ...
    $(window).scroll(function (event) {
      event.preventDefault();
      if ($(this).scrollTop() >= 200) $hHeader.addClass('scroll');else $hHeader.removeClass('scroll');
    }); // depth메뉴 보기 
  ...
  function quickSearchFn() {
    ...
    $quickSubmit.on('click', function (event) {
      event.preventDefault();
      alert("test : \uC120\uD0DD\uAC12 : ".concat($quickInput.val()));
      if (!$quickList.hasClass('hide')) $quickList.addClass('hide');
    });
  }
  ...
  function visualSlideFn() {
    ...
    var $slideLength, $slideWrapWidth, $slideWrapCenter;
    var intervalSlide; //슬라이드 이벤트 생성
    ...
    var setSlideCenter = function setSlideCenter() {
    ...
    var setSlideBtns = function setSlideBtns() {
    ...
    var shiftToRight = function shiftToRight() {
      $slide = $slideWrap.find('.slide');
      $slideWrap.css({
        'left': -$slideWrapCenter + $slide.innerWidth() + 'px'
      });
      $slide.first().detach();
    ...
    var shiftToLeft = function shiftToLeft() {
      $slide = $slideWrap.find('.slide');
      $slideWrap.css({
        'left': -$slideWrapCenter - $slide.innerWidth() + 'px'
      });
  ...
  function listTabFn() {
    ...
    $tabWrap.addClass($currentTab); //탭 버튼 이벤트
  ...
  function checkMailFn() {
    ...
    $mailCheckBtn.on('click', function (event) {
      event.preventDefault();
      if (!$mailCheckBtn.hasClass('loading')) $mailCheckBtn.addClass('loading'); //texst
  ...
  function checkAccFn() {
    ...
    $submit.on('click', function (event) {
      ...
      $accCheck.addClass('loading'); //test
  ...
  function acceptTermsFn() {
    ...
    $jCheckBox.on('click', function (event) {
      ...
      alert("test || checkbox value: ".concat($jCheckValue.attr('value')));
    });
  }
  ...
  function registerFormFileFn() {
    ...
    $thumbInput.on('change', function (event) {
      var $thumbHtml = ["<div class='input-wrap bx-round_s'><input type='text' readonly value='", "'><button class='btn btn_del' type='button'></button></div>"];
      thumbImg = event.target.files[0];
      if ($thumbInput.next('.input-wrap').length > 0) $thumbInput.next().find('input').val(thumbImg.name);else $thumbInput.after($thumbHtml[0] + thumbImg.name + $thumbHtml[1]);
    ...
    $imgsInput.on('change', function (event) {
      var $imgsHtml = ["<i class='num bx-round_s bg_n'>", "</i><div class='input-wrap bx-round_s'><input type='text' readonly value='", "'><button class='btn btn_del' type='button' data-index='", "'></button></div>"];
      imgList.push(event.target.files[0]);
      $imgsInput.next().append($imgsHtml[0] + imgList.length //라벨 넘버
      + $imgsHtml[1] + imgList[imgList.length - 1].name + $imgsHtml[2] + (imgList.length - 1) //버튼 데이터 속성
      + $imgsHtml[3]); //파일 삭제

      $('#images').next().find('.btn_del').off('click').on('click', function (event) {
        event.preventDefault();
        var delNum = $(this).data('index');
        var $fileWrap = $(this).parents('.file-wrap'); //파일 태그 삭제

        $fileWrap.children('i').eq(delNum).remove();
        $fileWrap.children('.input-wrap').eq(delNum).remove(); //이미지 리스트 업데이트

        var imgArr = imgList.slice(delNum + 1);
        imgList = imgList.slice(0, delNum).concat(imgArr); //태그 속성 업데이트

        for (var idx = delNum; idx < imgArr.length + delNum; idx++) {
          $fileWrap.children('i').eq(idx).text("".concat(idx + 1));
  ...
  function tabTermsFn() {
    ...
    $tabBtns.on('click', function (event) {
      event.preventDefault();
      if ($(this).hasClass('on')) return;else {
  ...
  function popupFn() {
    ...
    $popBtn.on('click', function (event) {
      event.preventDefault(); //팝업 열기

      var $targetData = $(this).data("popup");
      var $targetPop = $(".popup[data-popup=".concat($targetData, "]"));
      $targetPop.addClass('on'); //배경 처리

      var $contentHeight = $targetPop.find('.container').outerHeight();
      if ($contentHeight > $(window).innerHeight()) $targetPop.find('.dim').css('height', $contentHeight + 80); //팝업 position
  ...
  function zoomSlideFn() {
    ...
    var $slideLength, $slideWidth, $slideHeight, $slideWrapWidth;

    var setSlideNum = function setSlideNum() {
      var len = $slide.length;
      var cnt = 1;
      var $slideNum, $num, $nums;
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (cnt === 1) {
          ...
          $num = $slideNum.find('.num'), $nums = $slideNum.find('.nums');
          $num.text(cnt);
          $nums.text(len);
        }
      }
    ...
    var initSlide = function initSlide() {
      //resize
      $slide = $slideWrap.find('.slide'); //set slide

      $slideWidth = $(window).innerWidth(); // $slideHeight = $slideWidth * 0.8 > $(window).innerHeight()? $(window).innerHeight() :  $slideWidth * 0.8;

      $slideHeight = $slideWidth * 0.8;

      if ($slideHeight > $(window).innerHeight()) {
        $slideHeight = $(window).innerHeight();
        $slide.find('img').css('width', 'auto'); // $slide.find('.slide-num').css('left', $slide.find('img').width() + 'px');
      ...
      $slide.first().before($slide.last().clone());
      $slide.last().after($slide.first().clone()); //set SlideWrap
  ...
  function selectAddrFn() {
    ...
    $selectOption.on('click', function (event) {
      event.preventDefault();
      if ($(this).hasClass('selected')) return;else {
