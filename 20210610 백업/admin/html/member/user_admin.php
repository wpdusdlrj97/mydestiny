...
if (!$admin_session) {
    ...
    $qry_string = "SELECT * FROM admin_information where admin_email is not null";
    ...
    if ($search) {
        ...
        if ($option == "admin_name") {
            $qry_string = $qry_string . " && (admin_name LIKE '%$slash_search_zero%')";
        } elseif ($option == "admin_phone") {
            $qry_string = $qry_string . " && (admin_phone LIKE '%$slash_search_zero%')";
        } elseif ($option == "admin_email") {
            $qry_string = $qry_string . " && (admin_email LIKE '%$slash_search_zero%')";
        }
        for ($x = 1; $x < count($search_array); $x++) {
            $slash_search_array = $search_array[$x];
            if ($option == "admin_name") {
                $qry_string = $qry_string . " && (admin_name LIKE '%$slash_search_array%')";
            } elseif ($option == "admin_phone") {
                $qry_string = $qry_string . " && (admin_phone LIKE '%$slash_search_array%')";
            } elseif ($option == "admin_email") {
                $qry_string = $qry_string . " && (admin_email LIKE '%$slash_search_array%')";
            }
        }
        $qry_string = $qry_string . " ORDER BY admin_join_date DESC";
    } else {
        $qry_string = $qry_string . " ORDER BY admin_join_date DESC";
    }
    ...
    $admin_latest_date = array();
    while ($row = mysqli_fetch_array($qry)) {
        array_push($admin_email, $row['admin_email']);
        array_push($admin_name, $row['admin_name']);
        array_push($admin_phone, $row['admin_phone']);
        array_push($admin_join_date, date('Y.m.d', strtotime($row['admin_join_date'])));
        array_push($admin_latest_date, date('Y.m.d', strtotime($row['admin_latest_date'])));
                                ...
                                <form onsubmit="return false;">
                                    ...
                                    <?php for ($x = 0; $x < $count; $x++) { ?>
                                        <div class="check-wrap"><input type="checkbox" class="check" name="user_list"
                                                                       value="<?php echo $admin_email[$x] ?>"></div>
                                    <?php } ?>
                                ...
                                <table class="table">
                                    ...
                                    <?php for ($x = 0; $x < $count; $x++) { ?>
                                        <tr class="tr has-link"
                                            onClick="location.href='/admin/html/member/user_admin_detail.php?id=<?php echo $admin_email[$x]?>'">
                                            <td class="td t-num"><span
                                                        class="value"><?php echo $x + $page * 5 + 1 ?></span>
                                            </td>
                                            <td class="td t-id"><span class="value"><?php echo $admin_email[$x] ?></span>
                                            </td>
                                            <td class="td t-name"><span
                                                        class="value"><?php echo $admin_name[$x] ?></span>
                                            </td>
                                            <td class="td t-tel"><span
                                                        class="value"><?php echo format_phone($admin_phone[$x]) ?></span>
                                            </td>
                                            <td class="td t-date t-date_latest"><span class="value">
                                                    <?php
                                                    if ($admin_latest_date[$x]=='1970.01.01') {
                                                        echo '접속 기록 X';
                                                    } else {
                                                        echo $admin_latest_date[$x];
                                                    } ?>
                                                </span>
                                            </td>
                                            <td class="td t-date t-date_join"><span
                                                        class="value"><?php echo $admin_join_date[$x] ?></span></td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
                                ...
                                <?php if ($page > 0) { ?>
                                    ...
                                    <?php if ($option) { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='user_admin.php?page=<?php echo $page - 1 ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"></button>
                                    <?php } else { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='user_admin.php?page=<?php echo $page - 1 ?>'"></button>
                                    <?php } ?>
                                ...
                                if ($all_count > 0) {
                                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                                        if ($x == $page) {
                                            ...
                                            if ($option) { ?>
                                                <a class="num on"
                                                   onclick="location.href='user_admin.php?page=<?php echo $x ?>&option=<?php echo $option ?>&search=<?php echo $search ?>'"><?php echo $x + 1 ?></a>
                                            <?php } else { ?>
                                                <a class="num on"
                                                   onclick="location.href='user_admin.php?page=<?php echo $x ?>'"><?php echo $x + 1 ?></a>
                                            <?php }
    ...
    <script>
        ...
        function search() {
            ...
            if (select_search == '아이디') {
                location.href = "user_admin.php?option=admin_email&search=" + user_search_input;
            } else if (select_search == '실명') {
                location.href = "user_admin.php?option=admin_name&search=" + user_search_input;
            } else { //전화번호
                location.href = "user_admin.php?option=admin_phone&search=" + user_search_input;
            }
            ...
            if (user_list_delete_string == '') {
                ...
                if (confirm("선택하신 항목을 삭제하시겠습니까?")) {
                    ...
                    $.ajax({
                        ...
                        , data: {delete_type:'admin',delete_list: user_list_delete_string}
                        , success: function (result) {

                            if (result == 'success') {
                                alert("선택 항목을 삭제하였습니다");
                                location.href = "https://landmarking.co.kr/admin/html/member/user_admin.php";
                            } else {
