...
if (!$admin_session) {
    ...
    if($filter_service_status=='1'){ //대기중
        $qry_string = "SELECT * FROM pay_information where service_type NOT IN ('9') and service_status='1'";

    }else if($filter_service_status=='2'){ //진행중
        $qry_string = "SELECT * FROM pay_information where service_type NOT IN ('9') and service_status='2'";

    }else if($filter_service_status=='3'){  //완료
        $qry_string = "SELECT * FROM pay_information where service_type NOT IN ('9') and service_status='3'";

    }else{ //전체
        $qry_string = "SELECT * FROM pay_information where service_type NOT IN ('9')";
    }
                            ...
                            <div class="table-area l-fright">
                                <table class="table">
                                    ...
                                    <tr class="tr">
                                        ...
                                        <th class="th t-status has-btn">진행상황</th>
                                        <th class="th t-date">요청일</th>
                                    </tr>
                                    </thead>
                                    <tbody class="tbody">
                                    <?php for ($x = 0; $x < $count; $x++) { ?>
                                        <tr class="tr has-link"onClick="window.open('/admin/html/popup_service_status.php?id=<?php echo $pay_id[$x]?>','pop','width=900, height=400, top=200, left=200');">
                                ...
                                <?php if ($page > 0) { ?>
                                    ...
                                    <?php if ($option) { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='analysis_list.php?page=<?php echo $page - 1 ?>&option=<?php echo $option ?>&search=<?php echo $search ?>&filter_service_status=<?php echo $filter_service_status?>&filter_service=<?php echo $filter_service?>'"></button>
                                    <?php } else { ?>
                                        <button class="btn btn_prev"
                                                onclick="location.href='analysis_list.php?page=<?php echo $page - 1 ?>&filter_service_status=<?php echo $filter_service_status?>&filter_service=<?php echo $filter_service?>'"></button>
                                    <?php } ?>
                                ...
                                if ($all_count > 0) {
                                    for ($x = ((int)($page / 5) * 5); $x < ((int)($page / 5) * 5) + 5; $x++) {
                                        if ($x == $page) {
                                            ...
                                            if ($option) { ?>
                                                <a class="num on"
                                                   onclick="location.href='analysis_list.php?page=<?php echo $x ?>&option=<?php echo $option ?>&search=<?php echo $search ?>&filter_service_status=<?php echo $filter_service_status?>&filter_service=<?php echo $filter_service?>'"><?php echo $x + 1 ?></a>
                                            <?php } else { ?>
                                                <a class="num on"
                                                   onclick="location.href='analysis_list.php?page=<?php echo $x ?>&filter_service_status=<?php echo $filter_service_status?>&filter_service=<?php echo $filter_service?>'"><?php echo $x + 1 ?></a>
                                            <?php }
    ...
    <script>
        ...
        function search() {
            ...
            if (select_search == '아이디') {
                location.href = "analysis_list.php?option=buyer_id&search=" + pay_search_input+"&filter_service_status="+ service_status+"&filter_service="+ service;
            } else if (select_search == '실명') {
                location.href = "analysis_list.php?option=buyer_name&search=" + pay_search_input+"&filter_service_status="+ service_status+"&filter_service="+ service;
            } else { //주소
                location.href = "analysis_list.php?option=land_address&search=" + pay_search_input+"&filter_service_status="+ service_status+"&filter_service="+ service;
            }
                    ...
                    $.ajax({
                        ...
                        , success: function (result) {
                            ...
                            if (result == 'success') {
                                ...
                                location.href = "https://landmarking.co.kr/admin/html/analysis_list.php";
                            } else {
